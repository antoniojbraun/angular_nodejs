...
const passwordZodRules = z.string().min(6,{message:'Password must have at least 6 characteres.'}).max(100, {message:'Password must have maximum 100 characteres'}).regex(
...
export const loginController = async (req: Request, res: Response) => {
    ...
    if(user.get('status') !== 'active'){
        res.status(400).json({message:'Please confirm you email'})
    }
