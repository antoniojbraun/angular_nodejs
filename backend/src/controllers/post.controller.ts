...
import { addPostService, deletePostService, getAllPostsService, getPostByIdService, getPostBySlugService, updatePostService } from "../services/post.service";
...
export const updatePostController = async (req:Request, res:Response) => {
    ...
    const {title,content,id,categoryId,userId} = req.body;
    ...
    if(post.userId !== userId){
        res.status(404).json({message:"You are not authorized to update this post."})
        return
    }
    ...
    const updatedPost = await updatePostService(id,title,content,slug,categoryId)
...
export const deletePostController = async (req:Request, res:Response) => {
    ...
    if(post.userId !== userId){
        res.status(400).json({message: "User is not authorized to delete this post"})
        return
    }
